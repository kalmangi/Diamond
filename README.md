...
2.Each component includes JSX and CSS file.
3.The squares structure is used in one component "Table Row".
4.This component is reused inside Home component.
Logic
1.Math.random is used to select a random number of an array to ceil that number.
2.Glyphicons are used to append star from that number generated for each row.
3.On click of the block, condition check is done if it contains star, if it contains it displays star else it will clear the current block and highlight the block which contains the star.
